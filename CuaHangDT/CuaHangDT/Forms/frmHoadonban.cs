...
MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            string sql;
            sql = "select TenNV from tblNhanvien where MaNV = '" + cboMaNhanVien.Text + "'";
            txtTenNhanVien.Text = functions.GetFileValues(sql);
        }
        ...
        {
            string sql;
            sql = "select TenKH from tblKhach where MaKH = '" + cboMaKhachhang.SelectedValue + "'";
            txtTenKhachHang.Text = functions.GetFileValues(sql);
            sql = "select DiachiKH from tblKhach where MaKH = '" + cboMaKhachhang.SelectedValue + "'";
            txtDiaChi.Text = functions.GetFileValues(sql);
        }

        private void cboMaSanPham_TextChanged_1(object sender, EventArgs e)
        {
            string sql;
            sql = "select TenSP from tblSanpham where MaSP = '" + cboMaSanPham.Text + "'";
            txtTenSanPham.Text = functions.GetFileValues(sql);
            sql = "select Giaban from tblSanpham where MaSP = '" + cboMaSanPham.Text + "'";
            txtDonGia.Text = functions.GetFileValues(sql);
